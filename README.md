...
|region | string | none | yes | the appropriate AWS region for your account. For a full list of the available regions please visit this [page](https://docs.aws.amazon.com/general/latest/gr/s3.html) | Can be passed as environment variable or system property `XM_S3_REGION`. |
|bucket | string | none | yes | your S3 bucket name | Can be passed as environment variable or system property `XM_S3_BUCKET`. |
|endpointOverride | string | none | no | custom S3-compatible endpoint URL (e.g. `http://localhost:9000` for MinIO). When set, path-style access is automatically enabled. | Can be passed as environment variable or system property `XM_S3_ENDPOINT_OVERRIDE`. |
...
|-------------|-----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 17.x.x      | 5.0.0                 | 5.0.0 - Upgrade to BrXM version 17.x (Java 21, Spring Boot 4). Add `endpointOverride` config property and env var support for all five settings (`XM_S3_ACCESS_KEY`, `XM_S3_SECRET_KEY`, `XM_S3_BUCKET`, `XM_S3_REGION`, `XM_S3_ENDPOINT_OVERRIDE`). |
