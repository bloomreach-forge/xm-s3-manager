...
public class AwsS3ServiceImpl implements AwsS3Service {
    ...
    public List<S3ListItem> getList(final String prefix, final String query, final Set<String> viewRoles, final Set<String> excludeRoles) {
        ...
        final Set<String> excludePrefixes = excludeRoles == null ? java.util.Collections.emptySet() : excludeRoles.stream()
            .filter(java.util.Objects::nonNull)
            // remove everything up to and including the 'exclude.' token so we get e.g. 'foo.bar'
            .map(r -> r.replaceFirst("(?i)^.*?exclude\\.", ""))
            .filter(s -> !s.isEmpty())
            .map(String::toLowerCase)
            .map(s -> s.replace('.', '/'))
            .map(s -> s.endsWith("/") ? s : s + "/")
        ...
        final List<S3ListItem> objects = objectListing.contents().stream()
            .filter(s3Object -> !s3Object.key().equals(prefix))
            .filter(s3Object -> {
                final String keyLower = s3Object.key().toLowerCase();

                // Exclude precedence: if any exclude prefix matches this key, filter it out
                boolean excluded = excludePrefixes.stream().anyMatch(pref -> keyLower.equals(pref.substring(0, pref.length()-1)) || keyLower.startsWith(pref));
                 if (excluded) {
                     return false;
                 }
        ...
        final List<S3ListItem> folders = objectListing.commonPrefixes().stream()
            .map(CommonPrefix::prefix)
            .filter(prefixStr -> {
                final String folderKey = prefixStr.endsWith("/") ? prefixStr.substring(0, prefixStr.length() - 1) : prefixStr;
